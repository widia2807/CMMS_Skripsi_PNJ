...
function statusBadge(s){
    ...
    return `<span class="px-2.5 py-1 rounded-full text-xs font-semibold border ${map[s] || 'bg-slate-100'}">${s}</span>`;
}
...
async function approve(id){
    const source_branch_id = prompt('Masukkan ID cabang sumber:');
    await fetch('/api/borrowings/'+id+'/approve',{
        method:'POST',
        headers:{
...
async function reject(id){
    await fetch('/api/borrowings/'+id+'/reject',{
        method:'POST',
        headers:{Authorization:'Bearer '+token}
    });
    loadBorrowings();
...
async function markPicked(id){
    await fetch('/api/borrowings/'+id+'/picked',{
        method:'POST',
        headers:{Authorization:'Bearer '+token}
    });
    loadBorrowings();
...
async function markReturned(id){
    await fetch('/api/borrowings/'+id+'/returned',{
        method:'POST',
        headers:{Authorization:'Bearer '+token}
    });
    loadBorrowings();
